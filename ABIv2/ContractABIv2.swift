...
public struct ContractV2: ContractProtocol {
  ...
  public func method(_ method: String, parameters: [AnyObject], extraData: Data) -> EthereumTransaction? {
    return nil
  }
  
  public init?(_ abiString: String, at: EthereumAddress?) {
    self.address = at
    ...
    public var options: TransactionOptions = .default

    /// Init with json ABI
//    public init(_ abiString: String, at address: EthereumAddress? = nil) {
//        let abi = try JSONDecoder().decode([ABIv2.Record].self, from: abiString.data)
//        let abiNative = try abi.map({ (record) -> ABIv2.Element in
//            try record.parse()
//        })
...
//    public func deploy(bytecode: Data, args: Any..., extraData: Data = Data(), options: TransactionOptions?) throws -> EthereumTransaction {
...
//    }
    ...
    ///   - options: Transaction options
    /// - Returns: Transaction and its hash
//    public func deploy(bytecode: Data, parameters: [Any], extraData: Data = Data(), options: TransactionOptions?) throws -> EthereumTransaction {
